...
likes=[]
def func_aux(cases,df): 
   while cases:
      imatge,df,likes=tinder(imatge,likes,clic(imatge),df,df_concatenado)
      var_1 = df.iloc[imatge]['images']#la seguent
      if len(df)==0:
         cases=False
      passem(bool = True, var = var_1) #la seguent)

def passem(bool=False, var = None):
   if bool == False: 
      var = df.iloc[imatge]['images'][0]
   else: 
      var = var[0]
   return var
